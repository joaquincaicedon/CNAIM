...
function Chart() {
	...
	var main = function () {
	  ...
	  // Group data by x values, vertical columns (actualización: usar d3.group)
	  var groupedByX = Array.from(d3.group(data0, d => d.x), ([key, values]) => ({ key: key, values: values }));
  
	  // Group data by y values, horizontal rows
	  var groupedByY = Array.from(d3.group(data0, d => d.y), ([key, values]) => ({ key: key, values: values }));
	  ...
	  function addLegends() {
		var legendRects = legendsGroup
		  ...
		  .attr('x', (d, i) => (i * attrs.legend.width))
		  .attr('fill', d => d.color);
		...
		var legendTexts = legendsGroup
		  ...
		  .attr("dominant-baseline", "middle")
		  .text(d => d.name)
		  .attr('x', (d, i) => (i * attrs.legend.width) + attrs.legend.width / 2)
		  .attr('y', attrs.legend.height + attrs.legend.textVerticalDistance);
  
		legendsGroup.attr('transform', function () {
	  ...
	  function addMatrixRects() {
		groupedByY.forEach(function (row) {
		  row.values.forEach(function (record) {
			// add x axis rects
			var matrixRect = matrixGroup.append('rect')
